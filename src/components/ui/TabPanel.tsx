      ...
      <div style={{ padding: '16px 20px 14px', borderBottom: '1px solid rgba(0,255,255,0.09)',
        ...
        <div>
          ...
          <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono',monospace",
            ...
            {tab === 'events'    && `${events.length} ACTIVE STREAMS`}
...
/* ════════════════════════════════════════════════════════════
   EVENTS TAB
════════════════════════════════════════════════════════════ */
function EventsTab({ events }: { events: GlobeEvent[] }) {
...
/* ════════════════════════════════════════════════════════════
   ANALYTICS TAB
════════════════════════════════════════════════════════════ */
function AnalyticsTab({ events }: { events: GlobeEvent[] }) {
  const totalUsers = events.reduce((s, e) => s + parseUsers(e.users), 0);

  // Category breakdown
  const catMap: Record<string, number> = {};
  events.forEach(e => { catMap[e.category] = (catMap[e.category] || 0) + parseUsers(e.users); });
  const cats = Object.entries(catMap).sort((a, b) => b[1] - a[1]);
  const maxCat = cats[0]?.[1] || 1;

  // Hemisphere split
  const north = events.filter(e => e.lat >= 0).length;
  const south = events.length - north;
  const east  = events.filter(e => e.lng >= 0).length;
  const west  = events.length - east;

  const COLORS = ['#00FFFF', '#A855F7', '#4ade80', '#FFB648', '#f87171', '#60a5fa'];
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 18 }}>
        {[
          ['TOTAL USERS',  fmt(totalUsers), '#00FFFF'],
          ['LIVE EVENTS',  String(events.length),  '#A855F7'],
          ['NORTH HEMI',   `${north} events`,  '#4ade80'],
          ['SOUTH HEMI',   `${south} events`,  '#FFB648'],
        ].map(([label, val, color]) => (
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        {cats.length === 0
          ? <div style={{ color: 'rgba(255,255,255,0.25)', fontSize: 11, textAlign: 'center', padding: '12px 0' }}>No data</div>
          : cats.map(([cat, users], i) => (
          <div key={cat} style={{ marginBottom: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
              <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.65)' }}>{cat}</span>
              <span style={{ fontSize: 10, fontFamily: "'JetBrains Mono',monospace",
                color: COLORS[i % COLORS.length] }}>{fmt(users)}</span>
            </div>
            <div style={{ height: 5, background: 'rgba(255,255,255,0.06)', borderRadius: 99, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${(users / maxCat) * 100}%`,
                background: `linear-gradient(90deg, ${COLORS[i % COLORS.length]}, ${COLORS[(i+1) % COLORS.length]})`,
                borderRadius: 99, transition: 'width 0.6s ease', boxShadow: `0 0 8px ${COLORS[i % COLORS.length]}66` }}/>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: 'rgba(0,255,255,0.04)', border: '1px solid rgba(0,255,255,0.10)',
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          {[
            ['🌍 N. Hemisphere', north, events.length, '#4ade80'],
            ['🌏 S. Hemisphere', south, events.length, '#FFB648'],
            ['🌐 Eastern',       east,  events.length, '#00FFFF'],
            ['🌎 Western',       west,  events.length, '#A855F7'],
            ...
            <div key={String(label)}>
              ...
              <div style={{ height: 3, background: 'rgba(255,255,255,0.06)', borderRadius: 99,
                ...
                <div style={{ height: '100%',
                  width: `${total ? ((count as number) / (total as number)) * 100 : 0}%`,
...
function NetworkTab({ events }: { events: GlobeEvent[] }) {
  const totalUsers = events.reduce((s, e) => s + parseUsers(e.users), 0);
  const connections = events.length * (events.length - 1) / 2; // full mesh

  // Signal quality: based on number of active events
  ...
  return (
    <div>
      ...
      {/* Topology type */}
      <div style={{ background: 'rgba(138,43,226,0.06)', border: '1px solid rgba(138,43,226,0.15)',
        borderRadius: 10, padding: '14px 16px', marginBottom: 18 }}>
        <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono',monospace",
          color: 'rgba(168,85,247,0.6)', letterSpacing: '0.15em', marginBottom: 10 }}>TOPOLOGY</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
          {[
            ['Type',       'Full Mesh',   '#A855F7'],
            ['Protocol',   'WebSocket',   '#A855F7'],
            ['Latency',    '< 50ms',      '#4ade80'],
            ['Uptime',     '99.9%',       '#4ade80'],
          ].map(([k, v, c]) => (
            <div key={String(k)}>
              <div style={{ fontSize: 9, fontFamily: "'JetBrains Mono',monospace",
                color: 'rgba(255,255,255,0.3)', marginBottom: 3 }}>{k}</div>
